...
function checkAfk() {
  get(ref(db, 'game/players')).then((snap) => {
    snap.forEach((userSnap) => {
      if (userSnap.child("cord_x").val() == userSnap.child("cord_x_old").val() && userSnap.child("cord_y").val() == userSnap.child("cord_y_old").val()) {
        if (userSnap.child("afk_time").val() >= 600) {
          update(ref(database, `users/${userSnap.key}/game`), {
            online: false
          });
          update(ref(database, `game/players/${userSnap.key}`), {
            afk_time: 0
          });
        } else {
          update(ref(database, `game/players/${userSnap.key}`), {
        ...
        update(ref(database, `users/${userSnap.key}/game`), {
          cord_x_old: userSnap.child("cord_x").val(),
          cord_y_old: userSnap.child("cord_y").val(),
          afk_time: 0
        });
      }
    });
